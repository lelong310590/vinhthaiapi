        ...
        <div class="component-table">
            <div class="component-table-bar">
                @if ($permissions->contains('name','slider_create'))
                    <a href="{{route('lito::slider.create.get',['type'=>$type])}}" class="btn-outline text-uppercase component-table-bar-button">Thêm mới</a>
                @endif
                ...
                <table class="table table-hover">
                    ...
                    @forelse($data as $d)
                        <tr>
                            ...
                            <td>{{$d->name}}</td>
                            <td>{{$d->index}}</td>
                            <td width="150">{!! getStatus($d->status) !!}</td>
                            <td class="action" width="150">

                                <a href="{{route('lito::slider.edit.get',['id'=>$d->id,'type'=>$type])}}"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
